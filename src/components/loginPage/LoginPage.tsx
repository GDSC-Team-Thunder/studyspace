    ...
    return (
        <div className="flex justify-center items-center flex-row h-screen w-[97%]">
            <div className="flex flex-col justify-center items-center bg-slate-50 opacity-60 h-4/6 w-7/12 rounded-3xl">
                <input className="flex h-11 w-7/12 p-1 bg-slate-200 pl-3 rounded-[25px] m-2 text-black placeholder-black" 
                ...
                <div className="flex flex-col h-11 w-7/12 m-2">
                    <button className="flex my-2 px-3 items-center justify-center self-start" 
                    onClick={handleLogin}
                    disabled={username.length == 0 || password.length == 0}>{newUser ? "Create New Account" : "Login"}</button>
                    <span className="self-start mt-3 underline underline-offset-4"
